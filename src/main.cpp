         ...
         char *i_argv[])
{
  ...
  else if (i_argc == 3)
  {
    ...
    if (!(atoi(i_argv[2]) == 0 || atoi(i_argv[2]) == 1))
    {
      std::cerr << "invalid boolen input (has to be 0 or 1)" << std::endl;
      return EXIT_FAILURE;
    }

    l_use_roe_solver = atoi(i_argv[2]);
    if (l_use_roe_solver)
    {
  ...
  else
  {
    ...
    std::cerr << "  ./build/tsunami_lab N_CELLS_X BOOL_USE_ROE_SOLVER" << std::endl;
    std::cerr << "where N_CELLS_X is the number of cells in x-direction." << std::endl;
    std::cerr << "where BOOL_USE_ROE_SOLVER is the bool value that decides if the roe solver is to be used." << std::endl;
    std::cerr << "BOOL_USE_ROE_SOLVER defaults to 0." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  while (l_simTime < l_endTime)
  {
    ...
    std::cout << "do time step: " << l_timeStep << std::endl;
