...
TEST_CASE("Test the one-dimensional Supercritical1d setup.", "[Supercritical1d]")
{
  ...
  // x in [8,12]
  REQUIRE(l_supercritical_1d.getHeight(10, 0) == 0.13f);

  REQUIRE(l_supercritical_1d.getMomentumX(10, 0) == 0.18f);

  REQUIRE(l_supercritical_1d.getMomentumY(10, 0) == 0);

  REQUIRE(l_supercritical_1d.getBathymetry(10, 0) == -0.13f);

  // x in [0,25] AND x notin [8,12]
  REQUIRE(l_supercritical_1d.getHeight(4, 0) == 0.33f);

  REQUIRE(l_supercritical_1d.getMomentumX(4, 0) == 0.18f);

  REQUIRE(l_supercritical_1d.getMomentumY(4, 0) == 0);

  REQUIRE(l_supercritical_1d.getBathymetry(4, 0) == -0.33f);
  ...
  REQUIRE(l_supercritical_1d.getBathymetry(30, 0) == -0.33f);
}